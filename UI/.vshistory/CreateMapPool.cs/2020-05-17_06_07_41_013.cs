...
namespace GModMountManager.UI
{
    public partial class CreateMapPool : Form
    {
        ...
        private void btn_create_addon_Click(object sender, EventArgs e)
        {
            ...
            if (lst_maps.Rows.Count > 0)
            {
                ...
                foreach (var map in game.Maps)
                {
                    ...
                    CreateThumbnailOverlay(game.Name, map.Name, new Font("Segoe UI", 20, FontStyle.Regular), map.Order).Save(stream, ImageFormat.Png);
                    stream.Close();
                }
            }
        ...
        private static Image CreateThumbnailOverlay(string title, string mapname, Font font, int order = -1, Color? _textColor = null, Color? _backColor = null, int height = 256, int width = 256, Image baseImage = null)
        {
            ...
            if (order > -1) drawing.DrawString(order.ToString(), font, textBrush, height - 40, height - 40);
            drawing.Save();
            textBrush.Dispose();
            drawing.Dispose();
            return img;
